...
def ground_truth_loader(path):
    with open(path, 'rb') as f:
        img = Image.open(f)
        img_np = np.array(img)
        img_np[img_np == 208] = 255  # set robot start to free space
        img_np[img_np == 127] = 1    # align free space to robot belief
        img = Image.fromarray(img_np)
        return img
...
def random_bbox(config, batch_size, mask_batch_same=True):
...
def mask_image(x, bboxes, config, mask_type='hole'):
